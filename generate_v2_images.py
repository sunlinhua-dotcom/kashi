#!/usr/bin/env python3
"""Batch-generate v2 supplementary images via apiyi.com."""
import base64
import json
import os
import sys
...
PROMPTS_FILE = "/Volumes/ProjectEXF/kashi/output/image_prompts_v2.json"
OUTPUT_DIR = "/Volumes/ProjectEXF/kashi/output/images"
LOG_FILE = "/Volumes/ProjectEXF/kashi/output/images/_generation_v2_log.json"
CONCURRENCY = 4
RETRIES = 3
...
def generate(item: dict) -> tuple[str, bool, str]:
    ...
    body = {"model": MODEL, "prompt": item["prompt"], "n": 1, "size": item["size"]}
    ...
    for attempt in range(RETRIES):
        try:
            ...
            if "data" not in payload or not payload["data"]:
                raise ValueError(f"No data: {payload}")
            obj = payload["data"][0]
            if "b64_json" in obj:
                ...
                raise ValueError(f"No image: {list(obj.keys())}")
...
def main():
    ...
    total = len(items)
    print(f"📦 v2 supplementary {total} images", file=sys.stderr)
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as pool:
        ...
        for fut in as_completed(futures):
            ...
            status = "✅" if ok else "❌"
            print(f"[{done}/{total}] {status} {it['id']} {fname} — {msg} (t+{elapsed:.0f}s)",
                  file=sys.stderr)
    ...
    print(f"\n📊 SUMMARY: {ok_n}/{total} succeeded in {time.time()-start:.0f}s", file=sys.stderr)
